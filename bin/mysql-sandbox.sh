...
echo -e "\n### Installing mysql and gzip for the client tools"
yum install mysql gzip -y

# Start the sandbox
echo -e "\n### Starting the mysql-sandbox on port $MYSQL_PORT"
export SANDBOX_AS_ROOT=1
make_sandbox $BUILD_DIR/mysql-* -- --bind_address=0.0.0.0 --no_load_grants --no_confirm --db_user=$MYSQL_USER --db_password=$MYSQL_PASS --sandbox_port=$MYSQL_PORT --master --my_clause=binlog_format=row --my_clause=expire_logs_days=1
